...
def load_and_preprocess_data_with_sequences(df, target='Energy', val_ratio=0.1, test_ratio=0.10, input_seq_length=48, output_seq_length=24):
    ...
    val_features_scaled = feature_scaler.transform(val_df.drop(columns=[target]))
    test_features_scaled = feature_scaler.transform(test_df.drop(columns=[target]))
    ...
    def prepare_data(features, targets, input_seq_length, output_seq_length):
        X, y = [], []
        max_idx = len(features) - input_seq_length - output_seq_length + 1
    ...
    # Prepare the data
    X_train, y_train = prepare_data(train_features_scaled, train_target_scaled, input_seq_length, output_seq_length)
    X_val, y_val = prepare_data(val_features_scaled, val_target_scaled, input_seq_length, output_seq_length)
    X_test, y_test = prepare_data(test_features_scaled, test_target_scaled, input_seq_length, output_seq_length)
