...
class TemporalDeepExplainer:
    def __init__(self,
                 ...
                 activation_fn='relu'  # New parameter: 'gelu' (default) or 'relu'
                 ):
        ...
        self.activation_fn = activation_fn  # 'gelu' or 'relu'
    ...
    class TemporalExplainer(nn.Module):
        ...
        def __init__(self, time_steps, n_features, num_attention_heads=4, num_conv_layers=2,
                     num_filters=64, kernel_size=3, dropout_rate=0.2, activation_shrink=0.1, activation_fn='gelu'):
            super().__init__()
            self.num_conv_layers = num_conv_layers
            ...
            for _ in range(num_conv_layers):
                ...
                # Select activation based on activation_fn
...
def myshap(
    ...
    try:
        ...
        if xai_method == 'Kernel':
            ...
            explainer = shap.GradientExplainer(best_model, background_training)
            shap_values = explainer.shap_values(new_data)
            shap_48_10 = shap_values[0, :, :, 0]  # Extract first horizon (0th index)
            shap_df = pd.DataFrame(shap_48_10,
            ...
            try:
                loaded_explainer = TemporalDeepExplainer.load(
                    fastshap_dir,
            ...
            fastshap_dir = f"./Results/FastSHAP/{df_name}/"
            try:
                loaded_explainer = VITExplainer.load(
                    fastshap_dir,
                    filename=f"{model_type}_vit"
                )
            except FileNotFoundError:
                raise ValueError(f"No pre-trained VIT explainer found for {model_type}")
            shap_df = loaded_explainer.explain(new_data)
            baseline_pred = loaded_explainer.base_pred.item()
        else:
        ...
        if (xai_method != 'TDE') & (xai_method != 'VIT') & (xai_method != 'Gradient'):
...
class VITExplainer:
    def __init__(self,
                 ...
                 smoothness_lambda: float = 0.2,
                 # Diffusion Parameters (reuse same names for compatibility)
                 vit_dim: int = 128,
                 vit_depth: int = 4,
                 vit_heads: int = 4,
                 vit_mlp_dim: int = 512,
                 # Patch extraction parameters (kept for API compatibility)
                 patch_mode: str = "feature",  # "feature" or "window"
                 patches_per_feature: int = 1,  # used if patch_mode == "feature"
                 num_patches_time: int = None,  # used if patch_mode == "window"
                 num_patches_feature: int = None,  # used if patch_mode == "window"
                 # Sampling
        ...
        if patch_mode not in ["feature", "window"]:
            raise ValueError("patch_mode must be 'feature' or 'window'")
        self.patch_mode = patch_mode
        self.patches_per_feature = patches_per_feature
        self.num_patches_time = num_patches_time
        self.num_patches_feature = num_patches_feature
    ...
    # ------------------------------
    # Diffusion Explainer Components
    # ------------------------------
    class DiffusionExplainer(nn.Module):
        """
        Diffusion-inspired denoising network.
        For compatibility, we keep the same parameter names (patch_mode, etc.)
        although the diffusion process here ignores them.
        """
        def __init__(self, time_steps: int, n_features: int, dim: int, depth: int, heads: int, mlp_dim: int,
                     patch_mode: str, patches_per_feature: int = 1,
                     num_patches_time: int = None, num_patches_feature: int = None):
            super().__init__()
            self.time_steps = time_steps
            self.n_features = n_features
            self.dim = dim
            self.depth = depth
            # The diffusion model here is a simple denoising network.
            # We process the feature dimension at each time step.
            self.input_proj = nn.Linear(n_features, dim)
            self.blocks = nn.ModuleList([VITExplainer.ResidualBlock(dim) for _ in range(depth)])
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            ...
            h = self.input_proj(x_noisy)  # (B, T, dim)
            for block in self.blocks:
                h = block(h)
            out = self.output_proj(h)  # (B, T, F)
            return out
    ...
    # -------------------------------------------------------------
    def initialize(self, X_train: np.ndarray, model_predict_func, feature_names: list):
        self._validate_input(X_train)
        self._setup_core_components(X_train, model_predict_func, feature_names)
        self._train_fastshap(X_train)
        return self

    def explain(self, instance: np.ndarray) -> pd.DataFrame:
    ...
    # -------------------------------------------------------------
    def _train_fastshap(self, X_train: np.ndarray):
        d = self.n_features
        weights, probs = self._compute_shapley_kernel(d)
        loader = self._create_dataloader(X_train)
        ...
        for epoch in range(self.n_epochs):
            ...
            for X_batch, in loader:
                ...
                with autocast():
                    loss = self._process_batch(X_batch, d, weights, probs)
            ...
            if current_loss < best_loss - self.delta:
            ...
            if self.verbose:
                lr = optimizer.param_groups[0]['lr']
                print(f"Epoch {epoch+1:03d} | Loss: {current_loss:.4f} | LR: {lr:.2e}")
    ...
    def _process_batch(self, X_batch: torch.Tensor, d: int, weights: torch.Tensor, probs: torch.Tensor) -> torch.Tensor:
        batch_size = X_batch.size(0)
        masks = self._generate_shapley_masks(batch_size, d, probs)
        total_samples = masks.size(0)
        expand_factor = total_samples // (batch_size * self.samples_per_feature)
        X_paired = X_batch.repeat_interleave(expand_factor * self.samples_per_feature, dim=0)
        baseline_paired = self.baseline.unsqueeze(0).expand(total_samples, -1, -1)
        masked_inputs = X_paired * masks + baseline_paired * (1 - masks)
        ...
        mask_phi = (masks * phi).sum(dim=(1, 2))
        mse_loss = ((preds - self.base_pred - mask_phi) ** 2).mean()
        l1_reg = self.l1_lambda * phi.abs().mean()
        smooth_loss = self.smoothness_lambda * (phi.diff(dim=1) ** 2).mean()

        return mse_loss + l1_reg + smooth_loss

    # -------------------------------------------------------------
    # Helper Methods (unchanged)
    # -------------------------------------------------------------
    def _validate_input(self, X_train: np.ndarray):
        if X_train.ndim != 3:
            raise ValueError("Input must be 3D: (samples, time_steps, features)")
        if X_train.shape[0] < 10:
            raise ValueError("Need at least 10 samples for stable training")

    def _setup_core_components(self, X_train: np.ndarray, model_predict_func, feature_names: list):
        self.time_steps = X_train.shape[1]
        self.n_features = X_train.shape[2]
        self.feature_names = feature_names
        self.model_predict_func = model_predict_func
        ...
        # Instantiate the diffusion model instead of the ViT.
        self.explainer = self.DiffusionExplainer(
            time_steps=self.time_steps,
            n_features=self.n_features,
            dim=self.vit_dim,
            ...
            patches_per_feature=self.patches_per_feature,
            num_patches_time=self.num_patches_time,
            num_patches_feature=self.num_patches_feature
        ).to(self.device)

    def _generate_shapley_masks(self, batch_size: int, d: int, probs: torch.Tensor) -> torch.Tensor:
        n_features = self.n_features
        samples = batch_size * self.samples_per_feature
        k = torch.multinomial(probs, samples, replacement=True)
        k = torch.arange(1, n_features, device=self.device)[k]
        rand = torch.rand(samples, n_features, device=self.device)
        masks = (rand.argsort(dim=1) < k.unsqueeze(1)).float()
        masks = masks.unsqueeze(1).expand(-1, self.time_steps, -1)
        if self.paired_sampling:
            masks = torch.cat([masks, 1 - masks], dim=0)
        return masks

    def _compute_shapley_kernel(self, d: int):
        key = (d, self.device)
        if key in self._shapley_cache:
            return self._shapley_cache[key]
        k = torch.arange(1, d, device=self.device, dtype=torch.float32)
        log_binom = (torch.lgamma(torch.tensor(d + 1.0, device=self.device))
                     - torch.lgamma(k + 1.0)
                     - torch.lgamma(torch.tensor(d - k + 1.0, device=self.device)))
        weights = (d - 1) / (k * (d - k) * torch.exp(log_binom))
        probs = weights / weights.sum()
        self._shapley_cache[key] = (weights, probs)
        return weights, probs

    def _create_dataloader(self, X_train: np.ndarray) -> DataLoader:
    ...
    def _preprocess_input(self, instance: np.ndarray) -> torch.Tensor:
        if instance.ndim == 2:
            instance = instance[np.newaxis, :, :]
        return torch.FloatTensor(instance).to(self.device)

    def _create_shap_dataframe(self, shap_values: np.ndarray) -> pd.DataFrame:
        return pd.DataFrame(
            shap_values,
            columns=self.feature_names,
    ...
    def _get_model_predictions(self, inputs: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            preds = torch.FloatTensor(self.model_predict_func(inputs.cpu().numpy()))
            return preds.flatten().to(self.device)

    # -------------------------------------------------------------
    # Serialization Methods (unchanged)
    # -------------------------------------------------------------
    def save(self, path: str, filename: str = "explainer") -> bool:
        os.makedirs(path, exist_ok=True)
        try:
            state = {
                "explainer": self.explainer.state_dict(),
            ...
            torch.save(state, os.path.join(path, f"{filename}.pt"))
            if self.verbose:
                print(f"Model saved to {os.path.join(path, f'{filename}.pt')}")
            return True
        except Exception as e:
            print(f"Error saving model: {e}")
            return False
    ...
    @classmethod
    def load(cls, path: str, filename: str = "explainer", device=None):
        try:
            ...
            # Instantiate the diffusion model instead of the ViT.
            explainer.explainer = cls.DiffusionExplainer(
                time_steps=vit_config.get("time_steps", explainer.time_steps),
                n_features=vit_config.get("n_features", explainer.n_features),
                dim=vit_config.get("dim", 128),
                depth=vit_config.get("depth", 4),
                heads=vit_config.get("heads", 4),
                mlp_dim=vit_config.get("mlp_dim", 512),
                patch_mode=vit_config.get("patch_mode", "feature"),
                patches_per_feature=vit_config.get("patches_per_feature", 1),
                num_patches_time=vit_config.get("num_patches_time", None),
                num_patches_feature=vit_config.get("num_patches_feature", None)
            ).to(device)
            explainer.explainer.load_state_dict(state["explainer"])
            if explainer.verbose:
                print(f"Loaded explainer from {os.path.join(path, f'{filename}.pt')}")
