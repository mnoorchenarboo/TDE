"""
TDE & FastSHAP Training System v5

MASKING STRATEGIES:
- TDE: Window/Feature masking (temporal structure)
- FastSHAP: Element-wise (time×feature) masking (simple, no temporal structure)

ARCHITECTURE:
- TDE: Temporal convolutions + direct input connection + adaptive sparsity
- FastSHAP: Pure simple MLP (NO input_weight trick)
...
# ============================
# TDE NETWORK - WITH SPARSITY CONTROL
# ============================

class TemporalExplainerNetwork(nn.Module):
    """
    TDE with:
    1. Temporal convolutions (dilated)
    2. Direct input connection for input-dependence
    3. Learnable sparsity threshold
    """
    def __init__(self, time_steps, n_features, hidden_dim=128, n_conv_layers=2,
        ...
        # Temporal convolutions with dilation
        conv_layers = []
        in_ch = n_features
        ...
        # Direct input connection (ensures input-dependence)
    ...
    def forward(self, x, baseline=None):
        ...
        baseline: (time_steps, n_features)
        """
        # Conv pathway
        ...
        # Apply soft thresholding for sparsity (differentiable)
        # Values below threshold are pushed toward zero
...
# No tricks, just proper training to ensure input-dependence
# ============================

class FastSHAPNetwork(nn.Module):
    """
    Pure simple MLP for FastSHAP.
    
    Key to avoiding reliability=1.0:
    1. NO LayerNorm (it can normalize away input variance)
    2. Proper initialization (not too small)
    3. Sufficient network capacity
    
    The network learns: input -> SHAP values
    Different inputs WILL produce different outputs if trained properly.
    """
    def __init__(self, input_dim, hidden_dim=256, n_layers=2, dropout_rate=0.2):
        super().__init__()
        self.input_dim = input_dim
        
        # Simple MLP - NO LayerNorm, NO BatchNorm
        layers = []
        in_dim = input_dim
        ...
        # Proper initialization (not too small, not too large)
    ...
    def forward(self, x, baseline=None):
        ...
        baseline: (input_dim,) baseline values - used only for reference, not in computation
        """
        # Pure MLP forward pass
        h = self.mlp(x)
...
class TemporalDeepExplainer:
    """
    TDE Trainer with:
    - Window/Feature masking (temporal structure)
    - Sparsity control via threshold and L1
    - Target sparsity regularization
    """
    def __init__(self, n_epochs=100, batch_size=256, patience=5, verbose=True, min_lr=1e-6,
                 ...
                 kernel_size=3, dropout_rate=0.2, sparsity_threshold=0.01,
                 optimizer_type='adam', learning_rate=1e-3, window_size=6,
                 paired_sampling=True, samples_per_feature=2, masking_mode='window',
                 # Backwards compatibility
                 fidelity_lambda=0.0, num_attention_heads=4, num_filters=64,
                 softshrink_lambda=0.001, **kwargs):
    ...
    def _generate_window_masks(self, batch_size):
        """Window-based masking for temporal structure"""
    ...
    def _generate_feature_masks(self, batch_size):
        """Feature-based masking (all time steps for selected features)"""
    ...
    def _process_batch(self, X_batch, optimizer):
        ...
        # Sparsity targeting loss - penalize deviation from target sparsity
        current_sparsity = self._compute_sparsity(phi)
        sparsity_loss = self.sparsity_lambda * (current_sparsity - self.target_sparsity) ** 2
        
        loss = coalition_loss + eff_loss + smooth_loss + l1_loss + sparsity_loss
...
# ============================
# FASTSHAP TRAINER - PURE MLP
# ============================

class FastSHAPExplainer:
    """
    FastSHAP with:
    - Element-wise (time×feature) masking
    - Pure simple MLP (no tricks)
    - Proper training ensures different inputs -> different outputs
    """
    def __init__(self, n_epochs=100, batch_size=256, patience=5, verbose=True, min_lr=1e-6,
    ...
    def _generate_element_masks(self, batch_size, probs):
        """
        Element-wise masking: each element (time, feature) is independently masked.
        This is DIFFERENT from TDE's window/feature masking.
        """
        total = batch_size * self.samples_per_feature
        d = self.input_dim
    ...
    def _process_batch(self, X_batch_flat, probs, optimizer):
        ...
        # Forward pass - just pass input, no baseline needed in pure MLP
...
def generate_shap_heatmap(shap_vals, feature_names, output_path, method_name):
    ...
    try:
        ...
        ax.set_title(f'{method_name} SHAP')
...
def run_optimization(explainer_type, X_train, X_val, model_predict_func, feature_names,
    ...
    # Use versioned study name (v5)
    study_name = f"{explainer_type}_{model_name}_v5"
...
def get_user_inputs():
    print("\n" + "="*60)
    print("TDE & FastSHAP Training v5")
    print("="*60)
    print("TDE: Window/Feature masking + Sparsity control")
    print("FastSHAP: Element-wise masking + Pure MLP")
    print("="*60)
    ...
    print("\nExplainer: 0:TDE 1:FastSHAP 2:Both")
    explainer_choice = int(input("--> [0-2]: ").strip() or "2")
    
    window_size = int(input(f"\n--> Window [{DEFAULT_WINDOW_SIZE}]: ").strip() or DEFAULT_WINDOW_SIZE)
    n_trials = int(input(f"--> Trials [{DEBUG_N_TRIALS}]: ").strip() or DEBUG_N_TRIALS)
    n_test = int(input("--> Test samples [5]: ").strip() or "5")
...
def train_and_compare(primary_use, option_number, model_name, container,
    ...
    if 'tde' in explainer_types:
        ...
        logger.info(f"  [TDE] Loss={final_loss:.6f}")
    ...
    if 'fastshap' in explainer_types:
        ...
        fs.save(str(fs_dir))
        save_explainer_metadata(primary_use, option_number, model_name, 'FastSHAP', study.best_params, study.best_value,
                                 final_loss, len(X_train), time_steps, n_features, opt_time, train_time, n_trials,
        ...
        logger.info(f"  [FastSHAP] Loss={final_loss:.6f}")
...
def main():
    ...
    for model_name in config['models']:
        ...
        logger.info("="*60)
        logger.info(f"TDE & FastSHAP v5 - {model_name}")
        logger.info("="*60)
